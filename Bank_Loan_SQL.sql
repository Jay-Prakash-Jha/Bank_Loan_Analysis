...
#########################################
## KPI3 :
SELECT distinct(verification_status) , CONCAT('$', ROUND(SUM(total_pymnt)/1000000),'M (', ROUND(SUM(total_pymnt*100)/ (select SUM(total_pymnt) from finance_2)), '%)') AS "Total Payment"
FROM finance_1 f1 INNER JOIN finance_2 f2 
ON (f1.id = f2.id) 
